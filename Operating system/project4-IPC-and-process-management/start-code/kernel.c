...
static void initialize_pcb(pcb_t *p, pid_t pid, struct task_info *ti)
{
	...
	p->node_queue.prev = &p->node_queue;
	p->node_queue.next = &p->node_queue;
	p->node_queue.l = NULL;
	p->node_queue.c = NULL;
	p->node_queue.s = NULL;
	p->node_queue.b = NULL;
	p->node_queue.x = OTHERS;
...
static void init_syscalls()
{
    ...
    /* Initialize the Interrupt Descriptor Table IDT can contain up to 256
       ...
       in this project. In this project all
...
void init_idt(void)
{
    ...
    /* General function to make a gate entry. */
...
static int do_spawn(const char *filename)
{
	...
	queue_put(&ready_queue, &pcb[num].node);
	total_ready_priority += pcb[num].priority;
	return pid;
}
...
static int do_kill(pid_t pid)
{
	...
	// todo: fill this in
	int num, i;
	num = find_the_pcb(pid);
	if(pid == current_running->pid)
	{
		enter_critical();
		current_running->status = EXITED;
		unblock_wait_all(&pcb[num].wait_queue);
		unblock_info(&pcb[num]);
		unblock_mail(&pcb[num]);
		scheduler_entry();
		leave_critical();
	}
	if(num == -1)
...
static int do_wait(pid_t pid)
{
	...
	queue_put(&pcb[num].wait_queue, (node_t*) current_running);
	scheduler_entry();
	leave_critical();
	return 0;
}

int find_first_exited_pcb()
{
	int i;
	for(i = 0; i < NUM_PCBS; i++)
...
static pcb_t * unblock_wait_one(node_t * wait_queue)
{
    pcb_t *p;
...
void unblock_wait_all(node_t * wait_queue)
{
  while( unblock_wait_one(wait_queue) )
    {}
}
static pcb_t * unblock_info_one(node_t * wait_queue)
{
    pcb_t *p;
...
void unblock_info(pcb_t *ThePcb)
{
	if(ThePcb->node_queue.l != NULL)
	{
		lock_t *l;
		l = ThePcb->node_queue.l;
		ASSERT(disable_count);
		pcb_t *p = unblock_info_one(&l->wait_queue);
		if(NULL != p )
		{
			p->node_queue.l = l;
			p->waiting_for_lock = NULL;
	...
	if(ThePcb->node_queue.c != NULL)
	{
		condition_t *c;
		c = ThePcb->node_queue.c;
		ASSERT(disable_count);
		pcb_t *p = unblock_info_one( &c->wait_queue );
		if(NULL != p)
			p->node_queue.c = c;
	}
	if(ThePcb->node_queue.s != NULL)
	{
		semaphore_t *s;
		s = ThePcb->node_queue.s;
		pcb_t *p;
		if(s->value < 1 && !queue_empty(&s->wait_queue))
		{
			p = unblock_info_one(&s->wait_queue);
			p->node_queue.s = s;
		}		
		else
			s->value++;
	...
	if(ThePcb->node_queue.b != NULL)
	{
		barrier_t *b;
		b = ThePcb->node_queue.b;
		b->size--;
	}
}
