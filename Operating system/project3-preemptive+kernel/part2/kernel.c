 ...
 * otherwise it will be linked incorrectly, and you will
...
static void init_syscalls()
{
    ...
    /* Initialize the Interrupt Descriptor Table IDT can contain up to 256
	   ...
	   2.  The processor multiplies the vector by eight to create the 
	   offset into the IDT. 
       3. The processor reads the eight byte descriptor from the IDT entry. 
	   4. CS, EIP and EFlags are pushed on the stack. 
	   5. EFlags[IF] is clearedi to disable hardware interrupts. 
	   6.  The processor fetches the first instruction of the interrupt 
	   handler * (CS:EIP). 
       ...
       EFlags, EIP and CS 
	   10. The interrupted program resumes at the point
       of interruption. */
void init_idt(void)
{
    int i;
