...
void Parser::HandlePragmaAsCheck() {
  ...
  Sema::PragmaAsCheckKind Kind =
    static_cast<Sema::PragmaAsCheckKind>(
...
void PragmaAsCheckHandler::HandlePragma(Preprocessor &PP,
  ...
  if (Tok.isNot(tok::identifier)) {
    PP.Diag(Tok.getLocation(), diag::warn_pragma_as_check);
    return;
  }
  const IdentifierInfo *II = Tok.getIdentifierInfo();
  if (II->isStr("rule1")) {
    Kind = Sema::PACK_RULE1;
  }
  else if (II->isStr("rule2")) {
    Kind = Sema::PACK_RULE2;
  }
  else {
    PP.Diag(Tok.getLocation(), diag::warn_pragma_as_check);
    return;
  ...
  if (Tok.isNot(tok::eod)) {
    PP.Diag(Tok.getLocation(), diag::warn_pragma_extra_tokens_at_eol) << "asCheck";
    return;
  }
  ...
  new (Toks) Token();
  Toks[0].startToken();
  Toks[0].setKind(tok::annot_pragma_as_check);
  Toks[0].setLocation(AsCheckTok.getLocation());
