...
int main()
{
    ...
    for(i = 1; i <= row * col; i++)
    {
		if((i/col+i%col)%2 ==0)
		{
			capbility[1][1 + i] = 1;
			if(i <= col * (row -1))
				capbility[1 + i][1 + (i+col)] = 1;
			if(i % col != 0)
				capbility[1 + i][1 + (i+1)] = 1;
			if(i > col)
				capbility[1 + i][1 + (i-col)] = 1;
			if(i % col != 1)
				capbility[1 + i][1 + (i-1)] = 1;
		}
		else capbility[1 + i][1 + row*col + 1] = 1;
    }
    for(i = 1; i <= hole; i++)
    {
        scanf("%d%d", &j, &k);
		capbility[1][1 + (j*col-col+k)] = 0;
		capbility[1 + (j*col-col+k)][1 + row*col + 1]=0;
    }
    Head = 1; Tail = 1 + row*col + 1; Point = 1 + row*col + 1;
    MAXFLOW = 0;
    while(BFS())
    {
        memset(current, 0, sizeof(current));
