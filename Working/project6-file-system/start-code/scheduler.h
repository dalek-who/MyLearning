...
enum {
	...
	/*	interrupts enabled, I/O privilege level 0 (only kernel mode processes/
...
/*	The TRACE macro allows you to trace critical sections as they are
	...
	functions in C, and have enter/leave_critical call their C implementation.
	Take care to save the registers, though!)
*/
#define TRACE(s) \
	do { \
...
//	Prototypes
	...
	*/
	...
	/*	Remove current running from ready queue and insert it into 'q', and release
		<spinlock> (don't touch <spinlock> if it is 0!)
	*/
	...
	//	Read the directory from the floppy and copy it to 'buf'
	void	readdir(unsigned char *buf);

	//	Load a process from the floppy
