...
//	thread 2 waits until shared_var % 3 != 0 (cnot0mod3) until it uses shared_var 
void thread2(void) {
	int tmp, i;

	//	This thread has to initialize the locks and condition variables for the monitor 
	lock_init(&l);
	...
	*/
	...
	for (i = 0; i < 200; i++) {
		...
		/*	Wait until thread3 signals that shared_var % 3 != 0.
			Remember that condition_wait unlocks l.
			This allows thread2 to enter the monitor.
		*/
		while (shared_var % 3 == 0) {
			condition_wait(&l, &cnot0mod3);
		}
		ASSERT(shared_var % 3 != 0);
		/*	after beeing signaled, thread2 acquires the lock so it is safe 
			to use shared_var
		*/
		tmp = shared_var;
		if (i % 13 == 0) {
...
/*	thread 3 waits for condition shared_var % 3 != 0 (c0mod3)
	until it manipulates shared_var
*/
void thread3(void) {
	int tmp, i;
