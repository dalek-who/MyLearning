...
typedef struct {
    ...
    char links; // link数量
    char padding[INODE_PADDING];//对齐
} inode_t;
#define MAX_FILE_BLOCK (DIRECT_BLOCK_NUM + BLOCK_SIZE / sizeof(short))
#define MAX_FILE_SIZE (MAX_FILE_BLOCK * BLOCK_SIZE)
...
//64 
#define DIR_ENTRY_COUNT (BLOCK_SIZE / sizeof(file_entry_t))
typedef struct{
    bool_t is_open; // 是否打开
    int cursor; // 当前读/写位置
    short inode; // 文件Inode
...
#define MAX_FD_TABLE DATA_BLOCKS
//b=(i*n+d-1)/m +1
//i=d/n
//b+i+d=f
//m=BLOCK_BIT
...
//d = 1927 i = 120 b = 1
#define BIT_MAP_BLOCKS ((2 * FS_SIZE - 1) / BLOCK_BIT + 1)
//1
#define INODE_BLOCKS ((FS_SIZE - 3 - BIT_MAP_BLOCKS - 1) / (1 + INODE_COUNT) + 1)
//121
#define DATA_BLOCKS (FS_SIZE - 3 - BIT_MAP_BLOCKS - INODE_BLOCKS)
//1923
#endif
