...
typedef struct {
    ...
    condition_t	moreSpace,
				moreData;
    int			count,	//	Number of messages in mailbox
    			head,	//	Points to the first free byte in the buffer
    			tail;	//	points to oldest message (first to be recived) in buffer
...
//	Prototypes
	...
	*/ 
	...
	/*	Get number of messages (count) and number of bytes available 
		in the mailbox buffer (space). Note that the buffer is also used
		for storing the message headers, which means that a message 
		will take sizeof(mbox_t) + m->size bytes in the buffer. 
	*/ 
