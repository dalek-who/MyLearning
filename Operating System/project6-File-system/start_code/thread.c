...
/* 	xchg exchanges the given value with the value in *addr. 
	This means that this function will write LOCKED to *addr 
	and return the value that was in *addr. 
...
/*	Wait until lock is free, then acquire lock. Note that this isn't strictly
	a spinlock, as it yields while waiting to acquire the lock. This implementation
	is thus more "generous" towards other processes, but might also generate higher
	latencies on acquiring the lock.
*/
...
//	unlock m and block the thread (enqued on c), when unblocked acquire lock m
