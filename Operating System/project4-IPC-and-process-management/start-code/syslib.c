...
//we do begin
int lock_init_do(void)
{
    return invoke_syscall(SYSCALL_LOCK_INIT, IGNORE, IGNORE, IGNORE);
}
void lock_acquire_do(lock_id i)
{
    invoke_syscall(SYSCALL_LOCK_ACQUIRE, (int)i, IGNORE, IGNORE);
}
void lock_release_do(lock_id i)
{
    invoke_syscall(SYSCALL_LOCK_RELEASE, (int)i, IGNORE, IGNORE);
}
//we do end*/
