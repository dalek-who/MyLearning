...
#define REPLACE_TYPE 3
#define MAX 4
//other global variables...
lock_t page_lock;
page_map_entry_t page_queue;
int change = 0;
/* Main API */
...
int page_alloc(int pinned){
  ...
  {
    free_index = page_replacement_policy();
    ASSERT(free_index > 0);
    page_swap_out(free_index);
  }
  if(pinned == FALSE)
  {
    page_map[free_index].prev = page_queue.prev;
    page_map[free_index].next = &page_queue;
    page_queue.prev->next = &page_map[free_index];
    page_queue.prev = &page_map[free_index];
  }
  ...
  page_map[free_index].is_dirty = FALSE;
...
void init_memory(void){
  ...
  int i, j;
  lock_init(&page_lock);
  page_queue.next = &page_queue;
  page_queue.prev = &page_queue;
...
void setup_page_table(pcb_t * p)
{
  ...
  // process page tables
  for (vaddr = 0; vaddr < p->swap_size * SECTOR_SIZE; vaddr += (PAGE_N_ENTRIES - get_tab_idx(vaddr + p->start_pc) - 1) * PAGE_SIZE) 
  {
    index = page_alloc(TRUE);
    insert_ptab_dir(p->page_directory, page_map[index].addr, vaddr + p->start_pc, PE_US | PE_RW | PE_P);
    page_map[index].pid = p->pid;
...
void page_swap_out(int i){
  ...
  uint32_t dir_entry, entry;
  uint32_t *tab;
  dir_entry = page_map[i].page_dir[get_dir_idx(vaddr)];
  tab = (uint32_t *) (dir_entry & PE_BASE_ADDR_MASK);
  if(tab[get_tab_idx(vaddr)] & PE_D > 0)
    page_map[i].is_dirty = TRUE;
  entry = tab[get_tab_idx(vaddr)] & PE_BASE_ADDR_MASK;
  entry |= (PE_RW | PE_US) & ~PE_BASE_ADDR_MASK;
  tab[get_tab_idx(vaddr)] = entry;
  flush_tlb_entry(vaddr);
  page_map[i].prev->next = page_map[i].next;
  page_map[i].next->prev = page_map[i].prev;
  if (page_map[i].is_dirty) {
    scsi_write(get_disk_sector(&page_map[i]), page_map[i].swap_size, (char *)page_map[i].addr);
  }
}
...
int page_replacement_policy(void){
  int times, index;
  page_map_entry_t *node;
  if(page_queue.next == &page_queue)
    return -1;
  if(REPLACE_TYPE == 1)
    return page_queue.next->index;
  else if(REPLACE_TYPE == 2)
  {
    while(get_ptab_entry(page_queue.next->page_dir, page_queue.next->vaddr) & PE_A)
    {
      set_ptab_entry_flags(page_queue.next->page_dir, page_queue.next->vaddr, 
      (get_ptab_entry(page_queue.next->page_dir, page_queue.next->vaddr) & (~PE_A)));
      //A = prev, B = page_queue, C = next, D
      page_queue.next->prev = page_queue.prev;//C->prev = A 
      page_queue.prev->next = page_queue.next;//A->next = C
      page_queue.prev = page_queue.next;//B->prev = C
      page_queue.next = page_queue.prev->next;//B->next = D
      page_queue.next->prev = &page_queue;//D->prev = B
      page_queue.prev->next = &page_queue;//C->next = B
    } 
    return page_queue.next->index;
  }
  else if(REPLACE_TYPE == 3)//GG!
  {
    change++;
    times = MAX;
    index = 0;
    for(node = page_queue.next; node != page_queue.prev; node = node->next)
    {
      if(get_ptab_entry(node->page_dir, node->vaddr) & PE_A)
      {
        node->times++;
        set_ptab_entry_flags(node->page_dir, node->vaddr, 
        (get_ptab_entry(node->page_dir, node->vaddr) & (~PE_A)));
      }
      if(times > node->times)
      {
        times = node->times;
